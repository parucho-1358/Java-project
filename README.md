...
초기 List<Item> 구조에서 Inventory 클래스를 분리하고, 예외 처리를 위한 InvalidInputException을 추가하여 **단일 책임 원칙(SRP)**을 준수하는 구조로 리팩토링했습니다.

2. 상속과 인터페이스를 사용한 이유 (OOP)
...
초기 설계에서는 List<Item>을 사용했으나, 아이템 개수를 관리하고 검색하는 데 비효율적이었습니다.
...
해결: Inventory.java의 useItem() 메서드에서 items.get(itemName) - 1로 계산만 하고, items.put()으로 맵에 다시 저장하지 않은 논리적 오류를 발견하여 수정했습니다.

3. Git & SourceTree 푸시(Push) 오류
...
Eclipse IDE에서 File > Import > General > Existing Projects into Workspace를 선택합니다.

Select root directory에서 Clone 받은 Java-project 폴더를 선택하고 Finish를 누릅니다.

src/project/Game.java 파일을 찾아 Run As > Java Application으로 실행합니다.
